...
rm -f /tmp/histogram.dat #length.dat 
for n in {1..10000}; do 
	./collatz -n $n | wc -l >> /tmp/histogram.dat #length.dat
done 
cat /tmp/histogram.dat | uniq -c | less >> /tmp/histogram.dat #length.dat after cat  
