...
void ht_delete(HashTable **ht) {
    ...
    for (uint32_t index = 0; index < ht_size(*ht); index += 1) { // if there are any remaining nodes left in the hash table
...
Node *ht_lookup(HashTable *ht, char *word) {
    ...
    uint32_t count = 0;
    uint32_t index = hash(ht->salt, word) % ht->size; // Credit: Hashing Lecture and Eugene's section on 03_04_22
...
Node *ht_insert(HashTable *ht, char *word) {
    ...
    if (count < ht->size) { // parse through the whole hashtable
        ...
        if (strcmp(ht->slots[index]->word, word) == 0) { // if the word at the ht index is the same as the word you are trying to insert
...
void ht_print(HashTable *ht) {
    ...
    uint32_t index = 0;
    for (index = 0; index < ht->size; index += 1) { // if the index value is greater than the size of the hash table
...
HashTableIterator *hti_create(HashTable *ht) {
    ...
    HashTableIterator *hti = (HashTableIterator *) malloc(sizeof(HashTableIterator)); // allocate memory for the hash table iterator
    hti->table = ht;
    hti->slot = 0;
    return hti;
}
...
Node *ht_iter(HashTableIterator *hti) {
    ...
    while (hti->slot < hti->table->size) { // while the slot field of the iterator is less than the size of the hash table iterator
        if (hti->table->slots[hti->slot] != NULL) { // if the entire hash table has not been iterated over
            hti->slot += 1; // increment the slot field of the iterator by one
            return hti->table->slots[hti->slot - 1]; // return the pointer to the next valid entry in the hash table 
        }
        hti->slot += 1; // increment the slot field of the iterator by one
    }
    return NULL;
