...
void write_code(int outfile, Code *c) {
    ...
    if (code_get_bit(c, buffer_top) == true) {
        writebuffer[buffer_top] = c->bits[buffer_top / 8] & (1 << (buffer_top % 8)); // set the bit to 1 and buffer to buffer
        buffer_top += 1;
    }
    if (code_get_bit(c, buffer_top) == false) {
        writebuffer[buffer_top] = c->bits[buffer_top/ 8] & ~(1 << (buffer_top % 8)); // set the bit to 0 and buffer to buffer
        buffer_top+= 1;
    }
...
void flush_codes(int outfile){
    ...
    write_bytes(outfile, buffer, byte_index); // CITE: Eugene's section on 02_25_2022; zeros out the buffer
}
